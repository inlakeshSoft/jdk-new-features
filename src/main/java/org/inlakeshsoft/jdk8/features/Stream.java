package org.inlakeshsoft.jdk8.features;
...
import org.inlakeshsoft.jdk8.features.model.Person;
import org.inlakeshsoft.jdk8.features.model.Product;
...
public class Stream {
	...
	public Stream() {
		...
		people = Arrays.asList(p1, p2, p3, p4, p5);
		products = Arrays.asList(pr1, pr2, pr3, pr4);
	}
	...
	private void filter() {
		// (param: Predicate)
		List<Person> filteredList = people.stream().filter(p -> Stream.getAge(p.getBirthDate()) >= 18)
				.collect(Collectors.toList());
		Stream.printList(filteredList);
	}

	private void map() {
		// (param: Function)
		Function<String, String> coderFunction = name -> "Coder " + name;
		List<String> filteredList = people.stream().map(Person::getName).map(coderFunction)
				.collect(Collectors.toList());
		Stream.printList(filteredList);
	}
	...
	private void sorted() {
		...
		List<Person> sortedList = people.stream().sorted(byBirthDate).collect(Collectors.toList());
		Stream.printList(sortedList);
	}
	...
	private void match() {
		...
		boolean rpta1 = people.stream().anyMatch(startsWithPredicate);
		// allMatch : Evalua todo el stream bajo la condicion
		boolean rpta2 = people.stream().allMatch(startsWithPredicate);
		// noneMatch : Evalua todo el stream bajo la condicion
		boolean rpta3 = people.stream().noneMatch(startsWithPredicate);
	}

	private void limitSkip(int pageNumber, int pageSize) {
		List<Person> pagedList = people.stream().skip(pageNumber * pageSize).limit(pageSize)
				.collect(Collectors.toList());
		Stream.printList(pagedList);
	}

	private void collectors() {
		// GroupBy
		Map<String, List<Product>> collect1 = products.stream().filter(p -> p.getPrice() > 20)
	...
	private void reduce() {
		Optional<Double> sum = products.stream().map(Product::getPrice).reduce(Double::sum);
		// .reduce((a,b) -> a+b)
